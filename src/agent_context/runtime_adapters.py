...
def export_runtime_adapter_package(
    ...
    *,
    ...
    review_launch = export_runtime_review_launch(root, session_id, port=int(review_port))
    review_client = review_launch["review_client"]
    target_docs = {
    ...
    adapter_files = {
        ...
        "review_launch_manifest": review_launch["files"]["launch_json"],
        "review_launch_markdown": review_launch["files"]["launch_md"],
    }
    ...
    manifest = {
        ...
        "review_launch": {
            "manifest": review_launch["files"]["launch_json"],
            "markdown": review_launch["files"]["launch_md"],
            "review_server_url": review_launch["review_server_url"],
            "start_server_command": review_launch["start_server_command"],
            "open_client_command": review_launch["open_client_command"],
        },
...
def adapter_entrypoints(root: Path, session_id: str, *, agent_command: str, review_port: int) -> dict[str, str]:
    return {
        ...
        "agent_preflight_answer": doctor_command(root, "agent-preflight", "--session-id", session_id, "--advance", "answer", "--answer-command", quote_arg(agent_command)),
        "agent_preflight_execution": doctor_command(root, "agent-preflight", "--session-id", session_id, "--advance", "execution", "--execution-command", quote_arg("python -c \"print('runtime artifact')\"")),
        ...
        "export_review_launch": doctor_command(root, "runtime-review-launch", "--session-id", session_id, "--port", str(int(review_port))),
...
def mcp_tool_sequence(session_id: str, *, agent_command: str) -> list[dict[str, Any]]:
    return [
        ...
        {"tool": "doctor_agent_preflight", "arguments": {"session_id": session_id, "advance": "answer", "answer_command": agent_command}},
        {"tool": "doctor_answer_review", "arguments": {"session_id": session_id, "action": "approve", "reason": "answer matches intent"}},
        {"tool": "doctor_agent_preflight", "arguments": {"session_id": session_id, "advance": "execution", "execution_command": "python -c \"print('runtime artifact')\""}},
...
def render_target_doc(manifest: dict[str, Any], target: str) -> str:
    ...
    lines = [
        ...
        "- Use `doctor agent-preflight --advance answer` / `doctor_agent_preflight(advance='answer')` to prepare or run the answer gate.",
        "- Use `doctor agent-preflight --advance execution` / `doctor_agent_preflight(advance='execution')` to prepare or run the execution gate.",
        ...
        manifest["entrypoints"]["agent_preflight_answer"],
        manifest["entrypoints"]["agent_preflight_execution"],
